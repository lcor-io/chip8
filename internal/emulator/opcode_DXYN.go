...
func (e *emulator) opcode_DXYN(xRegister uint8, yRegister uint8, spriteHeight uint8) {
	...
	for row := range spriteHeight {
		...
		py_coordinate := e.Cpu.V[yRegister] + row

		for column := range SPRITE_LENGTH { // Loop on each column of the sprite at the current row

			px_coordinate := e.Cpu.V[xRegister] + column
			current_sprite_value := (spriteRow >> column) & 0b00000001 // Get the bit of the row at the column position
